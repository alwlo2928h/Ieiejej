...
BOT_TOKEN = os.getenv('BOT_TOKEN', '8675144844:AAEjtdaCLbjLbzR-9wV4-QrMDIfRKVOfb7k')
ADMIN_IDS = {int(x) for x in os.getenv('ADMIN_IDS', '8675144844').split(',') if x.strip().isdigit()}
...
def init_db():
    ...
    q('''CREATE TABLE IF NOT EXISTS stock(id INTEGER PRIMARY KEY AUTOINCREMENT,product_id INTEGER NOT NULL,file_id TEXT NOT NULL,file_name TEXT DEFAULT '',item_text TEXT DEFAULT '',sold INTEGER DEFAULT 0,sold_to INTEGER,sold_at TEXT,FOREIGN KEY(product_id) REFERENCES products(id))''')
    ...
    q('''CREATE TABLE IF NOT EXISTS deposits(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,amount REAL NOT NULL,trx_id TEXT DEFAULT '',status TEXT NOT NULL,created_at TEXT NOT NULL,payment_category_id INTEGER DEFAULT 0)''')
    ...
    deposit_cols = {r['name'] for r in c.execute("PRAGMA table_info(deposits)").fetchall()}
    if 'payment_category_id' not in deposit_cols:
        c.execute("ALTER TABLE deposits ADD COLUMN payment_category_id INTEGER DEFAULT 0")
    cols = {r['name'] for r in c.execute("PRAGMA table_info(stock)").fetchall()}
    if 'item_text' not in cols:
        c.execute("ALTER TABLE stock ADD COLUMN item_text TEXT DEFAULT ''")
    c.commit(); c.close()
        ...
        try:
            if r['item_text']:
                await ctx.bot.send_message(
                    ...
                    msg('product','Purchased Account',escape(r['item_text'])),
                    parse_mode='HTML'
                )
            else:
                await ctx.bot.send_document(
                    update.effective_user.id,
                    r['file_id'],
                    caption=f'{p["name"]} - purchased'
                )
        except Exception:
            log.exception('delivery')
    return True
...
async def deposit_start(update,ctx):
    ...
    ctx.user_data['state']='deposit_category'
    ctx.user_data['deposit_categories']={str(r['name']).strip():r['id'] for r in cats if str(r['name']).strip()}
    rows=[[KeyboardButton(str(r['name']).strip()) for r in cats[i:i+2]] for i in range(0,len(cats),2)]
    rows.append([KeyboardButton('Back')])
...
async def payment_category(update,ctx,rid):
    ...
    limit_text=' | '.join(lim) if lim else 'No deposit limit configured.'
    body=f'<b>Method: {escape(r["name"])}</b>\n\n{escape(r["details"] or "Payment details are not set yet.")}\n\n{limit_text}\n\nSend deposit amount.'
...
async def payment_name(ctx,rid):
    if not rid: return ''
    c=db(); r=c.execute('SELECT name FROM payment_categories WHERE id=?',(rid,)).fetchone(); c.close()
...
async def deposit_trx(update,ctx,text):
    ...
    c=db()
    # Prevent accidental reuse of the same transaction ID while a deposit is pending/already approved.
    duplicate=c.execute("SELECT id,status FROM deposits WHERE lower(trx_id)=lower(?) AND status IN ('pending','approved') LIMIT 1",(text.strip(),)).fetchone()
    if duplicate:
        c.close(); await update.message.reply_text(msg('error','Duplicate Transaction','This Transaction ID has already been submitted.'),parse_mode='HTML',reply_markup=back_kb()); return
    cur=c.execute('INSERT INTO deposits(user_id,amount,trx_id,status,created_at,payment_category_id) VALUES(?,?,?,?,?,?)',(update.effective_user.id,amount,text.strip(),'pending',now(),rid or 0)); did=cur.lastrowid; c.commit(); c.close()
    for aid in ADMIN_IDS:
        try: await ctx.bot.send_message(aid,msg('payment','New Deposit',f'User: <code>{update.effective_user.id}</code>\nMethod: <b>{escape(str((await payment_name(ctx,rid)) or "Unknown"))}</b>\nAmount: <b>{money(amount)} ৳</b>\nTRX: <code>{escape(text.strip())}</code>\nDeposit ID: <code>{did}</code>\n\nUse Pending Deposits in Admin Panel.'),parse_mode='HTML')
        except: pass
    ...
    if (doc.file_name or '').lower().endswith('.xlsx'):
        ...
        try:
            ...
            for row in ws.iter_rows(values_only=True):
                ...
                item=' | '.join(vals)
                c.execute('INSERT INTO stock(product_id,file_id,file_name,item_text) VALUES(?,?,?,?)',
                          (pid,doc.file_id,doc.file_name or 'xlsx',item))
                count+=1
...
async def balance_uid(update,ctx):
    ...
    c=db(); exists=c.execute('SELECT 1 FROM users WHERE user_id=?',(uid,)).fetchone(); c.close()
    if not exists: await update.message.reply_text('User not found.'); return
...
async def balance_amt(update,ctx):
    ...
    if add:
        ...
        current=c.execute('SELECT balance FROM users WHERE user_id=?',(uid,)).fetchone()
        if not current or float(current['balance']) < a:
            c.close(); await update.message.reply_text(msg('error','Insufficient Balance','User does not have enough balance to deduct that amount.'),parse_mode='HTML'); return
        c.execute('UPDATE users SET balance=balance-? WHERE user_id=?',(a,uid))
...
async def pending(update,ctx):
    c=db(); rows=c.execute('SELECT * FROM deposits WHERE status="pending" ORDER BY id DESC LIMIT 50').fetchall()
    payment_names={r['id']:r['name'] for r in c.execute('SELECT id,name FROM payment_categories').fetchall()}
    c.close()
    ...
    body='\n\n'.join([f'ID <b>{r["id"]}</b> | User <code>{r["user_id"]}</code> | <b>{money(r["amount"])} ৳</b> | Method: <b>{escape(payment_names.get(r["payment_category_id"],"Unknown"))}</b> | TRX <code>{escape(r["trx_id"])}</code>' for r in rows])
...
async def deposit_action(update,ctx):
    ...
    await q.answer(); action,did=q.data.split(':'); did=int(did); c=db(); r=c.execute('SELECT * FROM deposits WHERE id=? AND status="pending"',(did,)).fetchone()
    ...
    if action=='approve':
        ...
        c.execute('UPDATE deposits SET status="rejected" WHERE id=?',(did,)); c.commit()
        try: await ctx.bot.send_message(r['user_id'],msg('error','Deposit Rejected',f'Your deposit of <b>{money(r["amount"])} ৳</b> was rejected by admin.'),parse_mode='HTML')
        except Exception: pass
...
async def handle(update,ctx):
    ...
    if user_state=='deposit_category':
        rid=ctx.user_data.get('deposit_categories',{}).get(text)
        if rid:
            await payment_category(update,ctx,rid)
            return
    if user_state=='deposit_amount':
        await deposit_amount(update,ctx,text)
        return
    if user_state=='deposit_trx':
        await deposit_trx(update,ctx,text)
        return
